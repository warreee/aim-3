...
public class FilteringWordCount extends HadoopJob {
    ...
    static class FilteringWordCountMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        protected void map(Object key, Text line, Context ctx) throws IOException, InterruptedException {
            ...
            Pattern.compile(" ").splitAsStream(line.toString().replace(",", "")).map(String::toLowerCase).filter(l -> !getFilterList().contains(l)).collect(groupingBy(Function.identity(), counting())).forEach((word, count) -> writeToCtx(word, count, ctx));
        }

        private void writeToCtx(String word, Long val, Context ctx) {
            try {
                ctx.write(new Text(word), new IntWritable(val.intValue()));
